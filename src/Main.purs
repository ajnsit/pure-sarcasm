...
import PureScript.Backend.Optimizer.Codegen.Wasm (codegenModule, wasmModulePath)
import PureScript.Backend.Optimizer.Codegen.Wasm.Builder (basicBuildMain, externalDirectivesFromFile)
import PureScript.Backend.Optimizer.Codegen.Wasm.Foreign (wasmForeignSemantics)
...
wasmArgParser :: ArgParser Command
wasmArgParser =
  ArgParser.choose "command"
    ...
    <* ArgParser.flagHelp
    <* ArgParser.flagInfo [ "--version", "-v" ] "Show the current version of purs-backend-wasm." Version.version

parseArgs :: Effect (Either ArgParser.ArgError Command)
parseArgs = do
  cliArgs <- Array.drop 2 <$> Process.argv
  pure $ ArgParser.parseArgs "purs-backend-wasm"
    "A PureScript backend for wasm."
    wasmArgParser
    cliArgs
...
main cliRoot =
  ...
  buildCmd args = basicBuildMain
    ...
    , foreignSemantics: Map.union coreForeignSemantics wasmForeignSemantics
    , onCodegenBefore: do
        mkdirp args.outputDir
        writeTextFile UTF8 (Path.concat [ args.outputDir, "package.json" ]) wasmModulePackageJson
    ...
    , onCodegenModule: \build (Module coreFnMod) backendMod@{ name: ModuleName name } -> do
        ...
        unless (Set.isEmpty backendMod.foreign) do
          ...
          res <- attempt $ oneOf
            [ maybe empty (\dir -> copyFile (Path.concat [ dir, wasmModulePath backendMod.name ]) foreignOutputPath) args.foreignDir
  ...
  bundleCmd shouldInvokeMain bundleArgs args = do
    ...
    let
      wasmBuildArgs = Array.filter (not <<< String.null)
    ...
    if shouldInvokeMain then do
      spawnFromParentWithStdin "wasmbuild" wasmBuildArgs $ Just $ "import { main } from '" <> entryPath <> "'; main();"
    else
      spawnFromParentWithStdin "wasmbuild" (Array.snoc wasmBuildArgs entryPath) Nothing
...
wasmModulePackageJson :: String
wasmModulePackageJson = """{"type": "module"}"""
